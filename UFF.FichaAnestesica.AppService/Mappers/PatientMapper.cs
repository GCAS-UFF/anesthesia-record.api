...
namespace UFF.FichaAnestesica.Service.Mappers
{
    ...
    {
        public static List<Patient> Map(IEnumerable<PatientViewDto> viewData)
        {
        ...
        private static SurgicalCenter? GetOrCreateSurgicalCenter(PatientViewDto item, Dictionary<string, SurgicalCenter> cache)
        {
        ...
        private static Specialty? GetOrCreateSpecialty(PatientViewDto item, Dictionary<string, Specialty> cache)
        {
        ...
        private static void MapSurgery(Patient patient, PatientViewDto item, Dictionary<string, Specialty> specialties, Dictionary<string, SurgicalCenter> surgicalCenters)
        {
        ...
        private static void MapProcedure(Surgery surgery, PatientViewDto item)
        {
            if (string.IsNullOrWhiteSpace(item.ProcedureId))
                return;

            var exists = surgery.Procedures
                .Any(p => p.ExternalId == item.ProcedureId);
        ...
        private static Patient CreatePatient(PatientViewDto item, Dictionary<string, Unit> units)
        {
        ...
        private static Unit? GetOrCreateUnit(PatientViewDto item, Dictionary<string, Unit> cache)
        {

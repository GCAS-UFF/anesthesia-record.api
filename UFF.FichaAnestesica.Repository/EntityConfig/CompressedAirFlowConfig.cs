...
{
    public class CompressedAirFlowConfig : IEntityTypeConfiguration<CompressedAirFlow>
    {
        public void Configure(EntityTypeBuilder<CompressedAirFlow> builder)
        {
            builder.ToTable("compressed_air_flows");
            ...
            builder.HasOne(x => x.MonitoringRecord)
                .WithMany(x => x.CompressedAirFlows)
                .HasForeignKey(x => x.MonitoringRecordId)
                .OnDelete(DeleteBehavior.Cascade);

            builder.HasIndex(x => x.Date).HasDatabaseName("IX_compressed_air_flows_timestamp");
